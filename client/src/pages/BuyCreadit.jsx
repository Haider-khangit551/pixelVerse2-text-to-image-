...
const BuyCreadit = () => {
            ...
            handler: async function (response) {
                ...
                try {
                    ...
                    if (verifyRes.data.success) {
                        ...
                        loadCreditsData(); // optional: to reload user's credit balance
                    } else {
    ...
    return (
        <motion.div
            ...
            <div className='flex flex-wrap justify-center gap-6 textx-left '>
                {plans.map((item, index) => (
                    ...
                    >
                        ...
                        <button onClick={() => paymentRazorpay(item.id)} className='bg-gray-800 rounded-md w-full text-white mt-8 text-sm py-2.5 min-w-52 cursor-pointer'>{user ? 'Purchase' : 'Get Started'}</button>
                    </div>
                ))}
            </div>
